...
import { X, Download, Copy, Check, Key, Shield, Lock, CheckCircle2, Loader2 } from "lucide-react";
...
const DownloadExtensionModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Fetch download data after 3 seconds
      const fetchTimeout = setTimeout(async () => {
        try {
          const { data, error } = await supabase.functions.invoke('get-extension-download', {
            body: { licenseKey, extensionId }
          });
  ...
  }, [isOpen, licenseKey, extensionId]);

  const handleCopyPassword = () => {
    if (downloadData?.extractPassword) {
      navigator.clipboard.writeText(downloadData.extractPassword);
  ...
  const CurrentIcon = loadingMessages[currentMessageIndex]?.icon || Loader2;
                  ...
                  >
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
                      ...
                      <p className="text-[11px] text-white/40">
                        Use esta senha para extrair o arquivo ZIP
                      </p>
                    </div>
