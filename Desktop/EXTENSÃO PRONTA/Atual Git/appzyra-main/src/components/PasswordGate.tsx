...
const PasswordGate = ({ children }: PasswordGateProps) => {
  ...
  const storeUserData = (userData: Record<string, string | null>) => {
    if (userData.name) sessionStorage.setItem(STORAGE_KEYS.name, userData.name);
    if (userData.email) sessionStorage.setItem(STORAGE_KEYS.email, userData.email);
    if (userData.document) sessionStorage.setItem(STORAGE_KEYS.document, userData.document);
    if (userData.fingerprint) sessionStorage.setItem(STORAGE_KEYS.fingerprint, userData.fingerprint);
    if (userData.license_key) sessionStorage.setItem(STORAGE_KEYS.licenseKey, userData.license_key);
    if (userData.phone) sessionStorage.setItem(STORAGE_KEYS.phone, userData.phone);
  };
